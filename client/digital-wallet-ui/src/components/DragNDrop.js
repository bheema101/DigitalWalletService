...
class DragNDrop extends Component {
  ...
  fileTypeChecker = file => new Promise((resolve) => {
  ...
  onPreview(file, id, tuid) {
    ...
    axios({
      ...
      .then((response) => {
        ...
        if(response.headers["content-type"]==="image/png") {
          this.setState({preview:response.data, downloadFileType:response.headers["content-type"]});
        }
        else{
    ...
    reader.addEventListener('loadend', (e) => {
      ...
      this.setState({preview:text, downloadFileType:response.headers["content-type"]});
    });
          ...
          <FileDrop onDrop={(files, event) => this.onDropFile(files, event)}>
            {this.state.files.length > 0
              ...
              >
                {file}  
                <br />
              </div><button onClick={() => this.downloadFile(file, this.state.id, this.state.tuid)}>Download</button></div>
                ))
              : `Drop some files here!`}
              <br />
              <br />
              <br />
              {this.state.downloadFileType === "image/png"  ? <img src={URL.createObjectURL(this.state.preview)} style={{height:"300px", width:"300px"}}/>:
